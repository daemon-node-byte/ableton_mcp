...
_COMMAND_SPECS = [
    ...
    _spec("create_arrangement_audio_clip", "arrangement", required=("track_index", "file_path", "start_time"), result="{start_time, end_time, length, name, file_path}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12 locally on 2026-04-09 with an absolute file path on an audio track. The 2026-04-12 arrangement residual pass confirmed mutate application but did not yet confirm a clean clip-state undo/redo round-trip for this mutation in the disposable-track validator flow."),
    ...
    _spec("resize_arrangement_clip", "arrangement", required=("track_index", "length"), optional=("clip_index", "start_time"), result="{start_time, end_time, length}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12 locally on 2026-04-09 for MIDI arrangement clips. Requires length > 0 and exactly one selector. The 2026-04-12 arrangement residual pass confirmed mutate application but did not yet confirm a clean clip-state undo/redo round-trip for this mutation in the disposable-track validator flow."),
    _spec("move_arrangement_clip", "arrangement", required=("track_index", "new_start_time"), optional=("clip_index", "start_time"), result="{start_time, end_time, length, notes_restored}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12 locally on 2026-04-09 for MIDI arrangement clips only. Requires exactly one selector. Audio clip move remains intentionally unsupported. The 2026-04-12 arrangement residual pass confirmed MIDI mutate application but did not yet confirm a clean clip-state undo/redo round-trip for this mutation in the disposable-track validator flow."),
    ...
    _spec("duplicate_to_arrangement", "arrangement", required=("track_index", "slot_index"), optional=("start_time",), result="{ok, start_time, source_track_index, source_slot_index}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12 locally on 2026-04-09 with session-to-arrangement MIDI note round-trip verification. The 2026-04-12 arrangement residual pass confirmed mutate application but did not yet confirm a clean clip-state undo/redo round-trip for this mutation in the disposable-track validator flow."),
    ...
    _spec("load_instrument_or_effect", "device", required=("track_index",), optional=("device_name", "native_device_name", "target_index", "uri"), result="{ok, mode, device_count_before, device_count_after, ...}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12 locally on 2026-04-09 for built-in native instrument names and discovered built-in instrument, sounds preset, MIDI-effect, and audio-effect URIs, then revalidated on 2026-04-11 for top-level native device insertion metadata and on 2026-04-12 for the current third-party discovery limitation. Requires exactly one source. Native device_name/native_device_name insertion relies on Track.insert_device and is therefore limited to native Live devices. Third-party plugin URI loading is not currently discoverable through the validated normalized browser roots, so the confirmed URI path remains built-in content only."),
    ...
    _spec("get_rack_macros", "rack", required=("track_index", "device_index"), result="{rack_name, macros[]}", stability="confirmed", exposed=True, notes="{} Returns stable macro indices for read/write use. {}".format(RACK_MACRO_NOTE, USER_RACK_SEMANTICS_NOTE)),
    _spec("set_rack_macro", "rack", required=("track_index", "device_index", "macro_index", "value"), result="{name, value}", write=True, stability="confirmed", exposed=True, notes="{} Macro writes clamp to the parameter range.".format(RACK_MACRO_NOTE)),
    _spec("get_rack_structure", "rack", required=("track_index", "rack_path"), result="{track_index, rack_path, rack}", stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3+ locally on 2026-04-10 for recursive rack-tree serialization, then rechecked on 2026-04-12 during the imported-rack comparison against browser-loaded preset '808 Selector Rack.adg'. Recursively serializes chains, return_chains, nested devices, macros, and track-relative LOM-style paths. {}".format(USER_RACK_SEMANTICS_NOTE)),
    ...
    _spec("apply_rack_blueprint", "rack", required=("blueprint",), result="{rack_id, rack_path, created_racks, created_chains, created_devices, structure}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3+ locally on 2026-04-10 for deterministic system-owned rack generation and nested tuning, then rechecked on 2026-04-11 for the stable unsupported macro-authoring error path. The LOM audit found no documented native macro-to-parameter or macro-to-macro authoring API, so macro-mapping directives remain intentionally unsupported."),
    ...
    _spec("get_browser_items_at_path", "browser", optional=("path",), result="{items[], path}", stability="confirmed", exposed=True, notes="Validated in Ableton Live 12 locally on 2026-04-09 for top-level browser navigation, then extended on 2026-04-12 to confirm the normalized built-in roots include sounds presets in addition to instruments, drums, MIDI effects, and audio effects."),
    _spec("search_browser", "browser", optional=("query", "category"), result="{query, results[], count}", stability="confirmed", exposed=True, notes="Validated in Ableton Live 12 locally on 2026-04-09. Requires a non-empty query. Category-scoped searches are the confirmed path; broad category='all' plugin discovery can time out on the validated build."),
    ...
    _spec("get_take_lanes", "take_lane", required=("track_index",), result="{take_lanes[], available}", stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3.7 locally on 2026-04-12 on a disposable MIDI track. Returns available=true on the validated build and stable lane counts/clip counts."),
    _spec("create_take_lane", "take_lane", required=("track_index",), result="{ok, name, stability}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3.7 locally on 2026-04-12 using documented Track.create_take_lane() on a disposable MIDI track."),
    _spec("set_take_lane_name", "take_lane", required=("track_index", "lane_index", "name"), result="{name}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3.7 locally on 2026-04-12 for TakeLane.name mutation with stable lane index bounds checking."),
    _spec("create_midi_clip_in_lane", "take_lane", required=("track_index", "lane_index"), optional=("start_time", "length"), result="{start_time, end_time, length, name}", write=True, stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3.7 locally on 2026-04-12 using documented TakeLane.create_midi_clip(start_time, length). Requires a MIDI track, start_time >= 0, and length > 0."),
    _spec("get_clips_in_take_lane", "take_lane", required=("track_index", "lane_index"), result="{clips[]}", stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3.7 locally on 2026-04-12 for arrangement-clip readback on a disposable take lane after clip creation."),
    ...
    _spec("get_system_owned_racks", "memory_bank", result="{count, racks[]}", stability="confirmed", exposed=True, notes="Validated in Ableton Live 12.3+ locally on 2026-04-10 and re-read on 2026-04-12 after importing browser-loaded preset '808 Selector Rack.adg' with refresh_rack_memory_entry. Returns the parsed system-owned rack inventory tracked in racks.md, including explicitly imported rack entries."),
